#!/bin/bash
# Beetle Hardware Test for Twitching Body Animatronic
# Tests Arduino detection and code upload

set -e

echo "╔════════════════════════════════════════════════════════╗"
echo "║     🎃 BEETLE HARDWARE TEST - Twitching Body 🎃       ║"
echo "╚════════════════════════════════════════════════════════╝"
echo ""
...
if .pixi/bin/arduino-cli compile --fqbn arduino:avr:leonardo --config-file .arduino15/arduino-cli.yaml arduino/twitching_servos 2>&1; then
...
if .pixi/bin/arduino-cli upload -p $PORT --fqbn arduino:avr:leonardo --config-file .arduino15/arduino-cli.yaml arduino/twitching_servos 2>&1; then
...
echo "  2. Watch the serial output for behavior state changes"
echo "  3. Observe servos for movement patterns"
echo ""
